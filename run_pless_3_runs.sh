...
#SBATCH --output=slurm_out/pless_sweep_wm1_3_runs_%A_%a.out
#SBATCH --error=slurm_out/pless_sweep_wm1_3_runs_%A_%a.err
...
do
  ...
  python imagenet_arch_search.py --path pless_sweep_wm1_${SLURM_ARRAY_TASK_ID}_run_${RUN_NO} --dataset speech_commands --init_lr 0.2 --train_batch_size 100 --test_batch_size 100 --target_hardware "flops" --flops_ref_value 20e6 --n_worker 4 --gpu 0 --arch_lr 4e-3 --grad_reg_loss_alpha 1 --grad_reg_loss_beta ${PARRAY[$SLURM_ARRAY_TASK_ID]} --weight_bits 8 --width_mult 1 --n_mfcc 10 --manual_seed $SEED 
   
  sleep 60s
  python imagenet_run_exp.py --path pless_sweep_wm1_${SLURM_ARRAY_TASK_ID}_run_${RUN_NO}/learned_net --train --gpu 0 --manual_seed $SEED
